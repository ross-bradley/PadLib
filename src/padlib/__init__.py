...
class PadLibBase(object):
    '''
    Base class for exploiting CBC-mode padding oracles with PKCS#7 padding
    The async and sync versions share the functionality in this case
    '''

    CRIB_ENGLISH = 'etaoinshrdlcumwfgypbvkjxqz .,?!ETAOINSHRDLCUMWFGYPBVKJXQZ0123456789@#$%^&*()-_=+[]{}:;"\'<>~\t\r\n'
    CRIB_XML_SPARSE = ' \t<>"=/etaoinshrdlcumwfgypbvkjxqz0123456789:?![]ETAOINSHRDLCUMWFGYPBVKJXQZ@#$%^&*()-_+{}\'.,~\r\n'
    CRIB_XML_COMPACT = '<>"=/etaoinshrdlcumwfgypbvkjxqz0123456789 :?![]ETAOINSHRDLCUMWFGYPBVKJXQZ@#$%^&*()-_+{}\'.,~\t\r\n'
    CRIB_JSON_SPARSE = ' \t":{}[]etaoinshrdlcumwfgypbvkjxqz0123456789ETAOINSHRDLCUMWFGYPBVKJXQZ\r\n'
    ...
    def __init__(self, ciphertext, callback, block_size=16, iv=None, append_test_blocks=False, crib='', verbose=False):
        ...
        crib: a string of characters that are expected to appear in the plaintext, ordered from most-likely to less likely, OR, a function that takes the last plaintext byte and returns a crib
        ...
        # keep a note of the last plaintext byte we decrypted so the crib callback can use it to build a better crib
        #self.last_plaintext_byte = None
...
class PadLibAsync(PadLibBase):
    '''
    async version
    '''

    async def decrypt_block(self, idx, session):
        ...
        for byte_val in self.build_candidate_list(self.blocks[idx - 1][-1], 1):
            ...
            # query the oracle
            valid_padding = await self.callback(test_case, session)
            if valid_padding:
        ...
        self.last_plaintext_byte = plain_byte
        ...
        #print('DEBUG: ', (idx, plaintext))
        return (idx, bytes(plaintext))


    async def decrypt_block_MULTI(self, idx, session):
        # get the block to decrypt (c_1)
        c_1 = self.blocks[idx]
    ...
    async def decrypt(self):
        ...
        plaintext = [None for n in range(self.num_blocks)]

        async with AsyncHttp() as session:
            decrypters = [asyncio.create_task(self.decrypt_block(n, session)) for n in range(1, self.num_blocks)]
            results = await asyncio.gather(*decrypters)

        for result in results:
            plaintext[result[0]] = result[1]

        return {'plaintext': b''.join(plaintext[1:]), 'stats': self.stats}
