...
#def crib_wrapper(plaintext_byte):
#       return crib_wrapper.crib
...
class PadLib(object):
        ...
        def __init__(self, ciphertext, callback, block_size=16, iv=None, append_test_blocks=False, crib='', verbose=False):
                ...
                # if we're given a string, wrap it in an appropriate class, otherwise just store the object we were given
                # errors are a you problem...
                if isinstance(crib, str):
                        self.crib = crib_wrapper(crib)
                else:
                        self.crib = crib
        ...
        def build_candidate_list(self, ct_val, target):
                ...
                if self.state == STATE_FIND_PADDING:
                        ...
                        preferred = [ch for ch in self.crib.get_crib(self.last_plaintext_byte)]
